<form action="" method="post" id="form-add-brand" enctype="multipart/form-data">
    {{ csrf_field() }}
    <div class="box-body">
        ...
        <div class="form-group">
            ...
            <input class="form-control no-validate" id="site-link" placeholder="http://site_name.com" type="text" name="site_link" value="{{ old('site_link') }}">
        ...
        <div class="form-group" id="settings">
            ...
            <div class="checkbox">
                <label>
                    <input type="checkbox" class="no-validate" name="is_federal" value="1">
                    Федеральный бренд
                </label>
            </div>
            <div class="checkbox">
                <label>
                    <input type="checkbox" class="no-validate" name="is_internet_shop" value="1">
                    Интернет-магазин
                </label>
            </div>
        </div>
        <div class="form-group" id="categories">
            ...
            @foreach($categories as $category)
                <div class="checkbox">
                    <label>
                        <input type="checkbox" class="no-validate" name="{{ $category->code }}" value="{{ $category->id }}">
                        {{ $category->name }}
                    </label>
                </div>
            @endforeach
        ...
        <div class="form-group" id="cities">
            ...
            @foreach($cities as $city)
                <div class="checkbox">
                    <label>
                        <input type="checkbox" class="no-validate" name="{{ $city->code }}" value="{{ $city->id }}">
                        {{ $city->name }}
                    </label>
                </div>
            @endforeach
